...
-(void) viewDidLoad {
    ...
    if (self.immediateReturn) {
        ...
        UIBarButtonItem *doneButtonItem = [[UIBarButtonItem alloc] initWithTitle : @"Klaar" style : UIBarButtonItemStyleBordered target : self action : @selector(doneAction :)];
    ...
    NSString* minimg = [@(self.minimumimages) stringValue];
    NSString* numimg = [@(self.numimages) stringValue];
    NSString* txt = [NSString stringWithFormat : @"%@%s%@", numimg, " van ", minimg];
    ...
    myLabel.textColor = [UIColor redColor]; // change this color
...
-(void) setNumPhotos : (ELCAsset *) asset {
    ...
    if (self.numimages >= self.minimumimages) {
        self.navigationItem.rightBarButtonItem.enabled = true;
        myLabel.textColor = [UIColor colorWithRed:26/255.0f green:159/255.0f blue:81/255.0f alpha:1.0f]; // change this color
    } else {
